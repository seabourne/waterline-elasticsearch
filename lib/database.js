...
var createIndex = function(esclient, name, callback) {
  esclient.indices.create({
    ...
    callback(null, resp.acknowledged);
  });
};
...
var Database = module.exports = function(config, collections, cb) {
  ...
  if(config.requestTimeout)
    opts.requestTimeout = config.requestTimeout
  if(config.sniffOnStart)
    opts.sniffOnStart = config.sniffOnStart
  this.client = new elasticsearch.Client(opts);
  indexExists(self.client, self.config.index, function(err, exists) {
    if(err) return console.log('Error checking index', err)
    if (!exists) {
